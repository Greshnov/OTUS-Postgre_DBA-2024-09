...
```
\q
# Запрос на получение скрипта создания таблицы ticket_flights
...
CREATE TABLE bookings.ticket_flights_parted (
    ticket_no character(13) NOT NULL,
    flight_id integer NOT NULL,
    fare_conditions character varying(10) NOT NULL,
    amount numeric(10,2) NOT NULL,
    CONSTRAINT ticket_flights_parted_amount_check CHECK ((amount >= (0)::numeric)),
    CONSTRAINT ticket_flights_parted_fare_conditions_check CHECK (((fare_conditions)::text = ANY (ARRAY[('Economy'::character varying)::text, ('Comfort'::character varying)::text, ('Business'::character varying)::text])))
...
```
![image](https://github.com/user-attachments/assets/1c37cb52-701c-46e4-bbe6-80e7088721f4)
![image](https://github.com/user-attachments/assets/bc418e34-704e-4352-a8cf-f835e839bd59)
