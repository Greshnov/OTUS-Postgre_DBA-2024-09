...
Когда выполняется команда UPDATE без условия WHERE, она затрагивает все строки таблицы. Это означает, что транзакция пытается получить эксклюзивную блокировку на каждую строку таблицы для изменения их значений.
Если две транзакции одновременно начинают выполнять UPDATE без условия WHERE, они попытаются получить эксклюзивные блокировки на одни и те же строки. В результате:
- Первая транзакция может получить блокировку на часть строк.
- Вторая транзакция столкнётся с тем, что некоторые строки уже заблокированы первой транзакцией, и будет вынуждена ждать их освобождения.
